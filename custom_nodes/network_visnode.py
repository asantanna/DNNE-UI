...
class NetworkNode(RoboticsNodeBase):
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "checkpoint_trigger_type": (["epoch", "time", "best_metric", "end"], {"default": "epoch", "widget": {"name": "checkpoint_trigger_type"}, "tooltip": "When to save checkpoints: every N steps, time intervals, metric improvements, or on program end (Ctrl-C)"}),
    ...
    OUTPUT_DICT = {
        0: {"type": "LAYER_TENSOR", "name": "layers"},
        1: {"type": "NETWORK_OUTPUT_TENSOR", "name": "output"},
        2: {"type": "NETWORK_MODEL_OBJ", "name": "model", "virtual": True}  # Virtual - config only
    }
