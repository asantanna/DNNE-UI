...
- Update dnne-test telemetry to run ALL telemetry tests
...
- Fix critical bug: dnne-test telemetry only ran basic test due to set -e
- Remove set -e from dnne-test and commands.sh scripts
...
- Verify all 5 tests run via `./dnne-test telemetry` command
