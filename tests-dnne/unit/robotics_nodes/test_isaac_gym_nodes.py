...
# Isaac Gym requires special import order - check if torch is already imported
TORCH_ALREADY_IMPORTED = 'torch' in sys.modules

@pytest.mark.robotics
@pytest.mark.isaac_gym
def test_isaac_gym_availability():
    """Test Isaac Gym availability and import order."""
    if TORCH_ALREADY_IMPORTED:
        pytest.skip("Isaac Gym test skipped: torch was already imported by ML tests")
    
    try:
        import isaacgym
        from custom_nodes.robotics_nodes.isaac_gym_nodes import IsaacGymEnvNode, ISAAC_GYM_AVAILABLE
        
        assert ISAAC_GYM_AVAILABLE, "Isaac Gym should be available when imported in correct order"
        
    except ImportError as e:
        pytest.skip(f"Isaac Gym not available: {e}")


@pytest.mark.robotics  
@pytest.mark.isaac_gym
def test_isaac_gym_env_node_structure():
    """Test IsaacGymEnvNode basic structure when Isaac Gym is available."""
    if TORCH_ALREADY_IMPORTED:
        pytest.skip("Isaac Gym test skipped: torch was already imported by ML tests")
    ...
    try:
        ...
        if not ISAAC_GYM_AVAILABLE:
            pytest.skip("Isaac Gym not available")
        ...
        pytest.skip(f"Isaac Gym import failed: {e}")
...
def test_isaac_gym_step_node_structure():
    ...
    try:
        from custom_nodes.robotics_nodes.isaac_gym_nodes import IsaacGymStepNode, ISAAC_GYM_AVAILABLE
...
def test_or_node_structure():
    """Test OR node basic structure (doesn't require Isaac Gym)."""
    try:
        ...
        pytest.skip(f"OR node import failed: {e}")
...
def test_robotics_node_categories():
    ...
    try:
        # Import without Isaac Gym requirements
        from custom_nodes.robotics_nodes.isaac_gym_nodes import ORNode
        ...
        pytest.skip(f"Robotics node import failed: {e}")


@pytest.mark.robotics
def test_isaac_gym_import_order_awareness():
    """Test that Isaac Gym nodes are aware of import order issues."""
    # This test runs regardless of torch import status
    try:
        from custom_nodes.robotics_nodes.isaac_gym_nodes import ISAAC_GYM_AVAILABLE
        
        if TORCH_ALREADY_IMPORTED:
            # If torch was already imported, Isaac Gym should be marked as unavailable
            assert not ISAAC_GYM_AVAILABLE, \
                "Isaac Gym should be unavailable when torch is imported first"
        else:
            # If torch wasn't imported yet, Isaac Gym might be available
            # (depends on actual Isaac Gym installation)
            assert isinstance(ISAAC_GYM_AVAILABLE, bool), \
                "ISAAC_GYM_AVAILABLE should be a boolean"
