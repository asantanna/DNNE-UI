...
The main entry point is the `dnne-test` command in the project root, which provides access to all test categories:
...
dnne-test ml          # ML node tests only
dnne-test robotics    # Robotics/Isaac Gym tests
dnne-test export      # Export system tests
dnne-test rl          # RL comprehensive tests
```
...
Test dependencies are automatically installed when running tests via `dnne-test`. Manual installation:
