...
**Manual Client Test**:
```bash
cd dnne-test-suite/specialized/dnne_agent
...
- The automated test (`./dnne-test agent`) will check if the server is reachable
...
Tests use the exported_config.json for client configuration. Ensure the config files are properly set up in the dnne-agent directory.