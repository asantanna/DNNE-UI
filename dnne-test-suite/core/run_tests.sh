...
if [ ! -f "main.py" ] || [ ! -d "dnne-test-suite" ]; then
    echo "Error: Could not find project root directory"
    echo "Expected files: main.py, dnne-test-suite/"
...
run_test_category "Unit Tests" "dnne-test-suite/core/unit/" "15m" "30" || all_passed=false
