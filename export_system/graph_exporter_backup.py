...
class GraphExporter:
    ...
    def _generate_connections(self, links: List, nodes: List) -> List[str]:
        ...
        for node in nodes:
            ...
            elif node_type == "GetBatch":
                node_outputs[node_id] = ["batch_data", "batch_labels"]
            elif node_type == "SGDOptimizer":
                node_outputs[node_id] = ["optimizer"]
            elif node_type == "TrainingStep":
        ...
        for node in nodes:
            if str(node["id"]) == node_id:
                ...
                if node_type == "LinearLayer":
                    ...
                    return ["loss", "optimizer"][slot] if slot < 2 else f"input_{slot}"
                elif node_type == "RobotController":
                    return "action"
