...
Metadata: {
  "export_time": "2025-06-26 14:28:08",
  "workflow_id": "1",
  "workflow_name": "workflow_20250626_142808"
}
"""
...
class MNISTDatasetNode_22(QueueNode):
...
class BatchSamplerNode_23(QueueNode):
...
class GetBatchNode_24(SensorNode):
...
class LinearLayerNode_25(QueueNode):
    ...
    def __init__(self, node_id: str):
        ...
        # Create layer
        self.linear = nn.Linear(10, 128, bias=True)
...
class LinearLayerNode_26(QueueNode):
    """Linear layer with activation"""
...
class SGDOptimizerNode_28(QueueNode):
    """SGD Optimizer node"""
    
    def __init__(self, node_id: str, model_nodes: List[QueueNode]):
        super().__init__(node_id)
        self.setup_inputs(required=[])  # No inputs needed
        ...
        # Collect parameters from model nodes
        all_params = []
        for node in model_nodes:
            if hasattr(node, 'get_parameters'):
                all_params.extend(list(node.get_parameters()))
    ...
    async def run(self):
        """Override run to emit optimizer once"""
        self.running = True
        self.logger.info(f"Starting node {self.node_id}")
...
class TrainingStepNode_29(QueueNode):
...
async def main():
    ...
    # Create nodes
    node_22 = MNISTDatasetNode_22("22")
    node_23 = BatchSamplerNode_23("23")
    node_24 = GetBatchNode_24("24")
    node_25 = LinearLayerNode_25("25")
    node_26 = LinearLayerNode_26("26")
    node_27 = LossNode_27("27")
    node_28 = SGDOptimizerNode_28("28", [node_10, node_16])
    node_29 = TrainingStepNode_29("29")
    node_30 = NetworkNode_30("30")

    # Create runner
    runner = GraphRunner()

    # Add nodes to runner
    runner.add_node(node_22)
    runner.add_node(node_23)
    runner.add_node(node_24)
    runner.add_node(node_25)
    runner.add_node(node_26)
    runner.add_node(node_27)
    runner.add_node(node_28)
    runner.add_node(node_29)
    runner.add_node(node_30)

    # Wire connections
    connections = [
        ("22", "dataset", "23", "dataset"),
        ("22", "schema", "23", "dataset"),
        ("23", "dataloader", "24", "dataloader"),
        ("23", "schema", "24", "dataloader"),
        ("30", "layers", "25", "input"),
        ("25", "output", "26", "input"),
        ("30", "network_output", "27", "predictions"),
        ("24", "labels", "27", "predictions"),
        ("27", "loss", "29", "loss"),
        ("28", "optimizer", "29", "loss"),
        ("24", "images", "30", "input"),
        ("26", "output", "30", "input"),
    ]
