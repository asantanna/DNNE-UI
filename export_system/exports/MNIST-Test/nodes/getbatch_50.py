...
class GetBatchNode_50(QueueNode):
    ...
    async def run(self):
        ...
        try:
            ...
            self.logger.info(f"Received dataloader with {self.total_batches_per_epoch} batches per epoch, starting high-speed batch generation")
            
            # Generate batches continuously without rate limiting
            while self.running:
