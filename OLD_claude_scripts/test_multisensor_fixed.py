...
class CameraSensorNode_cam(SensorNode):
    ...
    def __init__(self, node_id: str):
        ...
        if self.use_real_camera:
            ...
            self.camera.set(cv2.CAP_PROP_FPS, 10.0)
...
class IMUSensorNode_imu(SensorNode):
    """IMU sensor that generates acceleration and gyro data"""
    
    def __init__(self, node_id: str):
        super().__init__(node_id, update_rate=50.0)
...
class DisplayNode_display_cam(QueueNode):
...
class DisplayNode_display_imu(QueueNode):
    """Display/logging node"""
...
async def main():
    ...
    # Create nodes
    cam_node = CameraSensorNode_cam("cam")
    imu_node = IMUSensorNode_imu("imu")
    display_cam_node = DisplayNode_display_cam("display_cam")
    display_imu_node = DisplayNode_display_imu("display_imu")

    # Create runner
    runner = GraphRunner()

    # Add nodes to runner
    runner.add_node(cam_node)
    runner.add_node(imu_node)
    runner.add_node(display_cam_node)
    runner.add_node(display_imu_node)

    # Wire connections
    connections = [
        ("cam", "image", "display_cam", "input_0"),
        ("imu", "output_0", "display_imu", "input_0"),
    ]
