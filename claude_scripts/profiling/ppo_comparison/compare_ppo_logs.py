...
Compares two PPO cycle debug logs side-by-side with:
- Line numbers on both sides
- Diff-based alignment for accurate comparison
- Color coding: white (exact match), yellow (match after preprocessing), red/green (differences)
- Ignores shared attribute differences by default (use --check-shared-attrib to include them)
"""
import re
import sys
import subprocess
...
class DiffBasedComparer:
    ...
    def display_alignment(self, alignment: List[Tuple[str, Optional[int], Optional[int]]],
        ...
        line_num_width = max(3, len(str(len(alignment))))
        line_num_space = line_num_width + 2  # number + ": "
        
        # Adjust column width to account for line numbers
        col_width = self.width - line_num_space
        
        # Header
        header1 = name1[:col_width].center(col_width)
        header2 = name2[:col_width].center(col_width)
        print(f"\n{' ' * line_num_space}{header1}{divider}{' ' * line_num_space}{header2}")
        print("=" * (line_num_space + col_width) + divider + "=" * (line_num_space + col_width))
        
        # Statistics
        stats = {'match': 0, 'yellow': 0, 'red': 0, 'green': 0}
        
        # Display each aligned pair with line numbers
        for line_num, (align_type, idx1, idx2) in enumerate(alignment, 1):
            # Format line numbers
            ...
            # Format for display
            formatted1 = line1[:col_width].ljust(col_width)
            formatted2 = line2[:col_width].ljust(col_width)
            
            if align_type == 'match':
                # White - exact match
                print(f"{line_num_str}{formatted1}{divider}{line_num_str}{formatted2}")
                stats['match'] += 1
                
            elif align_type == 'yellow':
                # Yellow - matches only after preprocessing
                print(f"{line_num_str}\033[33m{formatted1}\033[0m{divider}{line_num_str}\033[33m{formatted2}\033[0m")
                stats['yellow'] += 1
                ...
                if idx1 is not None and idx2 is not None:
                    ...
                    print(f"{line_num_str}\033[31m{formatted1}\033[0m{divider}{line_num_str}\033[32m{formatted2}\033[0m")
                    stats['red'] += 1
                    stats['green'] += 1
                elif idx1 is not None:
                    # Only left side (red)
                    print(f"{line_num_str}\033[31m{formatted1}\033[0m{divider}{line_num_str}{' ' * col_width}")
                    stats['red'] += 1
                else:
                    # Only right side (green)
                    print(f"{line_num_str}{' ' * col_width}{divider}{line_num_str}\033[32m{formatted2}\033[0m")
                    stats['green'] += 1
        
        # Summary
        print("\n" + "=" * ((line_num_space + col_width) * 2 + len(divider)))
        print(f"\nSummary:")
...
def main():
    ...
    parser.add_argument('--check-shared-attrib', action='store_true',
                        help='Check differences in shared attributes (D/I/B) when comparing lines (by default they are ignored)')
    ...
    comparer = DiffBasedComparer(width=args.width, ignore_shared_differences=not args.check_shared_attrib)
