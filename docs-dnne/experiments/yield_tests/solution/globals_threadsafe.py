...
class ThreadSafeYielder:
    ...
    async def start(self, loop: asyncio.AbstractEventLoop):
        ...
        if self.yielder_task is None:
            ...
            print(f"[THREAD_SAFE_YIELDER] Started yield processor in main loop")
    
    async def _yield_processor(self):
        """Process yield requests from threads"""
        print(f"[THREAD_SAFE_YIELDER] Yield processor running")
        ...
        while True:
            try:
                ...
                try:
                    ...
                    print(f"[THREAD_SAFE_YIELDER] Processing yield request from thread {thread_id}, delay={delay}")
                ...
                print(f"[THREAD_SAFE_YIELDER] Error in yield processor: {e}")
    ...
    def sync_yield_from_thread(self, delay: float = 0.05):
        ...
        print(f"[THREAD_SAFE_YIELDER] Thread {thread_id} requesting yield, delay={delay}")
        ...
        try:
            ...
            print(f"[THREAD_SAFE_YIELDER] Thread {thread_id} yield completed")
        except queue.Empty:
            print(f"[THREAD_SAFE_YIELDER] Thread {thread_id} yield timeout!")
...
def thread_safe_sync_adaptive_yield(delay: float = 0.5):
    ...
    try:
        ...
        print(f"[SYNC_YIELD] In async context, using direct yield")
        ...
        print(f"[SYNC_YIELD] In thread context, using thread-safe yield")
        
        # Use thread-safe yielder
        yielder = ThreadSafeYielder.get_instance()
        
        if yielder.main_loop is None:
            print(f"[SYNC_YIELD] Warning: No main loop registered, skipping yield")
            return
